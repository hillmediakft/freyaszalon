...
class Gyakori_kerdesek_model extends Site_model {
    ...
    /**
     * 	Egy gyakori kérdés minden adatát lekérdezi a részletek megjelenítéséhez
     */
...
// a query tulajdonság ($this->query) tartalmazza a query objektumot
    ...
    /**
     * 	Egy gyakori kérdés minden "nyers" adatát lekérdezi
     * 	A gyakori kérdés módosításához kell 
     */
    ...
    /**
     * 	A gyakori kérdések táblázathoz kérdezi le az adatokat
     * 	@return array
     */
    ...
    /**
     * 	Visszaadja az $id_array tömbben szereplő gyik-ek tömbjét 
     * 	
     *  @param array $id_array gyik-ek id-i
     * 	@return array a gyik-ek tömbje (üres is leet) 
     */
    public function getGyikById($id_array) {
        if (!empty($id_array)) {
            $this->query->reset();
            $this->query->set_table(array('gyik'));
            $this->query->set_columns('*');
            $this->query->set_where('gyik_id', 'in', $id_array);
            $this->query->set_join('left', 'gyik_categories', 'gyik.gyik_category_id', '=', 'gyik_categories.gyik_category_id');
            return $this->query->select();
        } else {
            return array();
    ...
    /**
     * 	Lekérdezi a gyakori kérdés kategóriákat a gyik_categories táblából (és az id-ket)
    ...
    /**
     * 	Lekérdezi a gyakori kérdéskategóriát a gyik_categories táblából 
     * 	@param	integer	$id  (ha csak egy elemet akarunk lekérdezni)
     * 	@return	array	
     */
...
//megnevezés ellenőrzése	
        if (empty($_POST['gyik_title'])) {
            $error_counter++;
            Message::set('error', 'A gyakori kérdés magyar megnevezése nem lehet üres!');
        }
        if (empty($_POST['gyik_category_id'])) {
            $error_counter++;
            Message::set('error', 'Választani kell egy kategóriát!');
...
//régi képek törlése
    ...
    /**
     * Termék (illetve gyakori kérdések) törlése
...
// default visszatérési érték (akkor tér vissza false-al ha hibás az sql parancs)	
     ...
     * Egy kategóriához tertozó gyakori kérdések számának meghatározásához kell
     * @return array
     */
    ...
    /**
     * 	gyakori kérdés képet méretezi és tölti fel a szerverre (thumb képet is)
...
// ha nincs hiba visszadja a feltöltött kép elérési útját
    ...
    /**
     * Ellenőrizzük, hogy a kategória törölhető-e: tartalmaz-e gyakori kérdést 	
     *
     * @param integer $id
...
// Nézőkép készítése
     ...
     * @return int a gyakori kérdések száma
     */
