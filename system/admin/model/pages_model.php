...
class Pages_model extends Admin_model {
	...
	public function all_pages()
	{
		...
		$this->query->set_columns(array('page_id', 'page_title', 'page_metatitle', 'page_metadescription', 'page_tags', 'page_friendlyurl'));
                $this->query->set_orderby('page_friendlyurl', 'ASC');
	...
	public function update_page($id)
	{
		...
		$data['page_metakeywords'] = $this->request->get_post('page_metakeywords');
                $data['page_tags'] = $this->request->get_post('page_tags');
	...
	public function page_data_query($id)
	{
		$this->query->reset();
		$this->query->set_table(array('pages'));
		$this->query->set_columns(array('page_id', 'page_title', 'page_body', 'page_friendlyurl', 'page_metatitle', 'page_metadescription', 'page_metakeywords', 'page_tags'));
		$this->query->set_where('page_id', '=', $id);
