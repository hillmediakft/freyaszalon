...
class Gyik_model extends Model {
    ...
    public function insert_gyik() {
        ...
        EventManager::trigger('insert_taxonomy', array($result, $terms, $this->content_type_id));
        Message::set('success', 'Gyakori kérdés sikeresen hozzáadva.');
        return true;
    }
    ...
    public function update_gyik($id) {
        ...
        if (empty($_POST['gyik_title'])) {
            $error_counter++;
            Message::set('error', 'A kérdés nem lehet üres!');
        }
        if (empty($_POST['gyik_category_id'])) {
            $error_counter++;
            Message::set('error', 'Választani kell egy kategóriát!');
        ...
        if ($error_counter == 0) {
            ...
            if ($result) {
                ...
                Message::set('success', 'Gyakori kérdés adatai sikeresen módosítva.');
                return true;
            }
        } else {
    ...
    public function delete_gyik() {
        ...
        if ($success_counter > 0) {
            EventManager::trigger('delete_taxonomy', array($content_id, $this->content_type_id));
            Message::set('success', $success_counter . ' gyik törlése sikerült.');
        }
        if ($fail_counter > 0) {
            Message::set('error', $fail_counter . ' gyik törlése nem sikerült!');
        }
