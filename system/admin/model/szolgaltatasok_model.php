...
class Szolgaltatasok_model extends Model {
    ...
    public function insert_szolgaltatas() {
        ...
        if ($error_counter == 0) {
            ...
            $data['slug'] = Util::string_to_slug($data['szolgaltatas_title']);

            // új adatok az adatbázisba
            $this->query->reset();
            $this->query->set_table(array('szolgaltatasok'));
            $last_inserted_id = $this->query->insert($data);
// $last_inserted_id - a létrehozott szolgáltatás id-je, $data['tags'] - term id-k, a címkék id-i, content_type - tartalom típusa - id
            EventManager::trigger('insert_taxonomy', array($last_inserted_id, $terms, $this->content_type_id));
    ...
    public function update_szolgaltatas($id) {
        ...
        if (isset($data['crew_members'])) {
            $data['crew_members'] = json_encode($_POST['crew_members']);
        } else {
            $data['crew_members'] = '';
        }        
    ...
    public function delete_szolgaltatas($id_arr) {
        ...
        EventManager::trigger('delete_taxonomy', array($id_arr, $this->content_type_id));
    ...
    public function ajax_get_szolgaltatasok($request_data) {
        ...
        if (isset($request_data['customActionType']) && isset($request_data['customActionName'])) {
            ...
            switch ($request_data['customActionName']) {
                ...
                case 'group_delete':
                    ...
                    if ($result['success'] > 0) {
                        $messages['success'] = $result['success'] . ' szolgáltatás sikeresen törölve.';
                    }
                    if ($result['error'] > 0) {
                        $messages['error'] = $result['error'] . ' szolgáltatás törlése nem sikerült, vagy nem törölhetők!';
                    }
                    break;
                ...
                case 'group_make_active':
                    ...
                    if ($result['success'] > 0) {
                        $messages['success'] = $result['success'] . ' szolgáltatás státusza aktívra változott.';
                    }
                    if ($result['error'] > 0) {
                        $messages['error'] = $result['error'] . ' szolgáltatás státusza nem változott meg!';
                    }
                    break;
                ...
                case 'group_make_inactive':
                    ...
                    if ($result['success'] > 0) {
                        $messages['success'] = $result['success'] . ' szolgáltatás státusza inaktívra változott.';
                    }
                    if ($result['error'] > 0) {
                        $messages['error'] = $result['error'] . ' szolgáltatás státusza nem változott meg!';
                    }
        ...
        foreach ($result as $value) {
            ...
            $temp['leiras'] = substr($value['szolgaltatas_info'], 0, 100) . '...';
            $temp['megtekintes'] = $value['megtekintes'];
            $temp['szolgaltatas_order'] = $value['szolgaltatas_order'];
            ...
            $temp['menu'] .= '<a class="btn btn-sm grey-steel" title="Műveletek" href="#" data-toggle="dropdown">
					...
					<ul class="dropdown-menu pull-right">
						<li><a href="' . $this->registry->site_url . 'admin/szolgaltatasok/view_szolgaltatas/' . $value['szolgaltatas_id'] . '"><i class="fa fa-eye"></i> Részletek</a></li>';

            // update
            $temp['menu'] .= ($user_role < 3) ? '<li><a href="' . $this->registry->site_url . 'admin/szolgaltatasok/update_szolgaltatas/' . $value['szolgaltatas_id'] . '"><i class="fa fa-pencil"></i> Szerkeszt</a></li>' : '';
    ...
    public function category_insert() {
        ...
        $data['szolgaltatas_list_name'] = trim($this->request->get_post('szolgaltatas_list_name'));
        $data['szolgaltatas_list_slug'] = Util::string_to_slug($_POST['szolgaltatas_list_name']);
        $data['szolgaltatas_list_desc'] = $this->request->get_post('szolgaltatas_list_desc');
...
//        $this->query->debug(true);
    ...
    public function category_update($id) {
        $data['szolgaltatas_list_name'] = trim($this->request->get_post('szolgaltatas_list_name'));
        $data['szolgaltatas_list_slug'] = Util::string_to_slug($this->request->get_post('szolgaltatas_list_name'));
        $data['szolgaltatas_list_desc'] = $this->request->get_post('szolgaltatas_list_desc');
        
        // régi képek elérési útjának változókhoz rendelése (ezt használjuk a régi kép törléséhez, ha új kép lett feltöltve)
        $old_img_name = $this->request->get_post('old_img');
    ...
    public function category_list_query() {
        $this->query->reset();
        $this->query->set_table(array('szolgaltatasok_list'));
        $this->query->set_columns(array('szolgaltatas_list_id', 'szolgaltatas_list_name', 'szolgaltatas_list_order'));
...
//a kép neve (ezzel tér vissza a metódus, ha nincs hiba!)
                    ...
                    $errors[] = Message::set($handle->error);
                    continue;
// return false;
                }
...
// beírjuk az adatbázisba
    ...
    /**
     * 	Szolgáltatás kategória sorrend módosítás
     */
    public function szolgaltatas_category_sort($list) {
        ...
        foreach ($list as $key => $value) {
            ...
            $data = array('szolgaltatas_list_order' => $order);
     ...
     * 	Szolgáltatás kategória sorrend módosítás
     */
    public function szolgaltatas_sort($list) {
        ...
        foreach ($list as $key => $value) {
            ...
            $data = array('szolgaltatas_order' => $order);
